...
The Dagster API documentation uses a **dual-pathway system** to determine which symbols appear in the public documentation. Understanding this system is crucial for docstring validation and maintaining API documentation quality.
...
**Key characteristic**: These directives **bypass the `@public` annotation system entirely**. If a symbol is explicitly listed in an RST file, it will be documented regardless of whether it has the `@public` annotation.
...
**Scope limitation**: This filtering does NOT affect:
- Top-level functions documented with `autofunction`
- Decorators documented with `autodecorator`
- Classes themselves when documented with `autoclass`
...
### TODO: API Documentation Consistency Improvements

The current dual-pathway system creates inconsistencies that complicate tooling and validation. The following improvements would create a more predictable and maintainable documentation system:

1. **Enforce `@public` decorator requirement**: Make the `@public` decorator mandatory on all symbols that are referenced in RST directives. This would enable tools to inspect a symbol locally and definitively determine whether its docstring will appear in public-facing documentation, without needing to parse RST files.

2. **Add enforcement testing**: Write automated tests to validate that all symbols documented via RST autodoc directives (`autodecorator`, `autofunction`, `autoclass`) are properly marked with the `@public` annotation. This test should fail the build if undocumented public symbols are found.

3. **Remove `dagster_pipes` special case**: Eliminate the special treatment of `dagster_pipes` modules in the Sphinx extension. All symbols, regardless of module, should follow the same `@public` annotation requirements for consistency.

These changes would create a unified system where the `@public` annotation serves as the single source of truth for determining public API surface area, making both documentation generation and validation tools more reliable and predictable.
