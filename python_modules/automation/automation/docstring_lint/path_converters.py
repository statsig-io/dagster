...
def dagster_path_converter(file_path: Path, root_path: Path) -> str | None:
    ...
    try:
        ...
        if parts and parts[0] == "dagster":
            ...
            # If the actual package directory name matches the converted lib name,
            # skip the first directory to avoid duplication (e.g., dagster-shared -> dagster_shared)
            if remaining_parts and remaining_parts[0] == lib_name:
                module_parts = [lib_name] + remaining_parts[1:]
            else:
